########################################################
#####       Author: Diego Valle Jones
#####       Website: www.diegovalle.net
#####       Date Created: Wed Nov 10 19:43:32 2010
########################################################
#Summary statistics of homicde

source("app/summary-stats-functions.R")
source("app/label-charts.R")

#Stats for all of Mexico
#Only presumed homicides

llcharts <- generateCharts(hom, kmaxy, "México")
llcharts$weekly <- labelChart(llcharts$weekly, "Start of Drug War",
                              300, op.mich)
llcharts$monthly <- labelChart(llcharts$monthly, "Start of Drug War",
                              1100, op.mich)
saveCharts(llcharts, "mexico")


hom.narco <- subset(hom, ABBRV %in% c("BC", "Chih", "Gro", "Mich", "Son",
                                  "Sin", "Dgo"))
llcharts <- generateCharts(hom.narco, kmaxy, "Narco States")
saveCharts(llcharts, "narco-states")



hom.border <- subset(hom, ABBRV %in% c("BC", "Chih", "Son",
                                  "Tamps", "NL", "Coah") 
                     )

x <- formatWeekly(formatDaily(hom.border))
x$group <- "all"
hom.border2 <- subset(hom, ABBRV %in% c("Son",
                                   "NL", "Coah") 
                     )
x2 <- formatWeekly(formatDaily(hom.border2))
x2$group <- "non-tam"
hom.border <- rbind(x, x2)
ggplot(subset(hom.border, date < as.Date("2006-12-01")),
       aes(date, V1, group = group, color = group)) +
       geom_line() +
  geom_smooth()
llcharts <- generateCharts(hom.border, kmaxy, "Border States")
llcharts$monthly
saveCharts(llcharts, "border-states")


edomex <- subset(hom, ABBRV == "Mex")
llcharts <- generateCharts(edomex, kmaxy, "State of Mexico")
saveCharts(llcharts, "edomex")
#year 2008
ddply(edomex, .(ANIODEF), nrow)[5,2] / 15175862*10^5
#Neza 058
ddply(subset(edomex, MUNOCU == 058 & EDADVALOR %in% c(15:34) & SEXOtxt == "Male"), .(MUNOCU, ANIODEF), nrow)
68 / 182404*10^5
ddply(subset(edomex, MUNOCU == 058 & EDADVALOR %in% c(15:34) & SEXOtxt == "Female"), .(MUNOCU, ANIODEF), nrow)
6/189494*10^5

df <- subset(hom, ABBRV == "DF")
llcharts <- generateCharts(df, kmaxy, "Federal District")
saveCharts(llcharts, "df")
ddply(df, .(ANIODEF), nrow)[5,2] / 8851080*10^5
#Iztapalapa = 007
ddply(subset(df, MUNOCU == 007 & EDADVALOR %in% c(15:34) & SEXOtxt == "Male"), .(MUNOCU, ANIODEF), nrow)
120/309438*10^5
ddply(subset(df, MUNOCU == 007 & EDADVALOR %in% c(15:34) & SEXOtxt == "Female"), .(MUNOCU, ANIODEF), nrow)
8/316007*10^5


204/1815786*10^5

#Femicides
fem <- subset(hom, SEXO == 2)
llcharts <- generateCharts(fem, kmaxy, "México (Females)")
saveCharts(llcharts, "femicide")

fem.j <- subset(hom, SEXO == 2 & ENTOCU == 08 & MUNOCU==037)
llcharts <- generateCharts(fem.j, kmaxy, "Juárez (Females)")
saveCharts(llcharts, "juarez-femicide")

hom.j <- subset(hom, ENTOCU == 08 & MUNOCU %in% c(037))
llcharts <- generateCharts(hom.j, kmaxy, "Juárez")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2008-03-28")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2008-03-28"), 60,
                label = "J.O. Chihuahua"),
            hjust = 1.03, vjust = 0) +
  geom_vline(aes(xintercept = as.Date("2009-03-01")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2009-03-01"), 80,
                label = "Reinforcements\nSent"),
            hjust = 1.03, vjust = 0) 
saveCharts(llcharts, "juárez (ma)")
#chartRegion(hom, c(08), c(037), kmaxy,
 #           "Juárez (MA)")
hom.chi <- subset(hom, MA == "Chihuahua")
llcharts <- generateCharts(hom.chi, kmaxy, "Chihuahua (MA)")
llcharts$monthly <- llcharts$monthly +
  geom_vline(aes(xintercept = as.Date("2008-03-28")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2008-03-28"), 60,
                label = "J.O. Chihuahua"),
            hjust = 1.03, vjust = 0) 
  
saveCharts(llcharts, "Chihuahua (MA)")

#chartRegion(hom, c(08), c(2, 4, 19), kmaxy,
 #           "Chihuahua (MA)")
chartRegion(hom, c(08), c(029), kmaxy,
            "Guadalup y Calvo")
chartRegion(hom, c(08), c(032), kmaxy,
            "Hidalgo del Parral")
chartRegion(hom, c(08), c(17), kmaxy,
            "Cuauhtemoc")

hom.can <- subset(hom, MA == "Cancún")
llcharts <- generateCharts(hom.can, kmaxy, "Cancún")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2009-02-09")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2009-02-09"), 6,
                label = "J.O. Cancún"),
             hjust = 1.03, vjust = 0) 
saveCharts(llcharts, "cancun")
#chartRegion(hom, c(23), c(005), kmaxy,
 #           "Cancún")

hom.col <- subset(hom, MA == "Colima-Villa de Álvarez")
llcharts <- generateCharts(hom.col, kmaxy, "Colima")
saveCharts(llcharts, "colima")

hom.col <- subset(hom, ABBRV == "Col")
llcharts <- generateCharts(hom.col, kmaxy, "Colima")
saveCharts(llcharts, "colima-state")

hom.nacho <- subset(hom, ABBRV %in% c("Jal", "Col", "Nay"))
llcharts <- generateCharts(hom.nacho, kmaxy, "Colima")
saveCharts(llcharts, "nacho-states")



hom.tj <- subset(hom, MA == "Tijuana")
#hom.tj <- subset(hom, ENTOCU == 02 & MUNOCU %in% c(004))
llcharts <- generateCharts(hom.tj, kmaxy, "Tijuana (MA)")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2007-01-03")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-01-03"), 30,
                label = "J.O. Tijuana"),
             hjust = 1.03, vjust = 0) +
  geom_vline(aes(xintercept = as.Date("2008-10-26")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2008-10-26"), 70,
                label = "E.A.F. Captured"),
             hjust = 1.03, vjust = 0)
llcharts$weekly <- labelChart(llcharts$weekly, "Teo\'s Girlfriend\nKilled",
                              82, "2008-11-29", -0.03)
saveCharts(llcharts, "tijuana")
#chartRegion(hom, c(02), c(004, 003, 005), kmaxy,
 #           "Tijuana (MA)")  
#write.csv(ddply(subset(hom, ENTOCU == 08 & MUNOCU %in% c(37)), .(ANIODEF, MESDEF), nrow), file = "temp.csv")
daily1 <- ddply(hom.tj, .(ANIODEF, MESDEF, DIADEF), nrow)
daily1 <- subset(daily1, ANIODEF %in% c(2008, 2009))
daily1$date <- with(daily1, as.Date(str_c(ANIODEF, MESDEF, DIADEF,
                                        sep = "-")))
ggplot(daily1, aes(date, V1)) +
  scale_x_date(format = "%b", minor = "month") +
  facet_wrap(~ANIODEF, nrow = 2, scales = "free_x") +
  opts(title = "Daily Homicides in Tijuana (MA)") +
  ylab("number of homicides") +
  geom_vline(data = data.frame(x = as.Date("2008-10-26"), ANIODEF = 2008) ,
                        aes(xintercept = x), alpha = .7,
               linetype = 2) +
  geom_text(data = data.frame(x = as.Date("2008-10-26"), ANIODEF = 2008) ,
                        aes(x, 21,
                            label = "Eduardo Arellano Félix\ncaptured"),
            hjust = 1.03, vjust = 0, size = 3.5) +
  geom_vline(data = data.frame(x = as.Date("2008-11-30"), ANIODEF = 2008) ,
                        aes(xintercept = x), alpha = .7,
               linetype = 2) +
  geom_text(data = data.frame(x = as.Date("2008-11-29"), ANIODEF = 2008) ,
                        aes(x, 15,
                            label = "\"El Teo\'s\"\ngirlfriend\nkilled & Truce"),
            hjust = -.15, vjust = 0, size = 3.5) +
  geom_vline(data = data.frame(x = as.Date("2009-12-07"), ANIODEF = 2009) ,
                        aes(xintercept = x), alpha = .7,
               linetype = 2) +
  geom_text(data = data.frame(x = as.Date("2009-12-07"), ANIODEF = 2009) ,
                        aes(x, 20,
                            label = "Truce ends?"),
            hjust = 1.03, vjust = 0, size = 3.5) +
  geom_vline(data = data.frame(x = as.Date("2008-04-26"), ANIODEF = 2008) ,
                        aes(xintercept = x), alpha = .7,
               linetype = 2) +
  geom_text(data = data.frame(x = as.Date("2008-04-26"), ANIODEF = 2008) ,
                        aes(x, 20,
                            label = "Shootout between the gunmen\n of \"El Teo\" and \"El Ingeniero\""),
            hjust = 1.03, vjust = 0, size = 3.5) +
  geom_vline(data = data.frame(x = as.Date("2008-09-29"), ANIODEF = 2008) ,
                        aes(xintercept = x), alpha = .7,
               linetype = 2) +
  geom_text(data = data.frame(x = as.Date("2008-09-29"), ANIODEF = 2008) ,
                        aes(x, 17,
                            label = "Sixteen executions"),
            hjust = 1.03, vjust = 0, size = 3.5) +
  geom_vline(data = data.frame(x = as.Date("2009-03-02"), ANIODEF = 2009) ,
                        aes(xintercept = x), alpha = .7,
               linetype = 2) +
  geom_text(data = data.frame(x = as.Date("2009-03-02"), ANIODEF = 2009) ,
                        aes(x, 20,
                            label = "\"El Profe\"\ncaptured"),
            hjust = 1.03, vjust = 0, size = 3.5)  +
  geom_vline(data = data.frame(x = as.Date("2008-09-16"), ANIODEF = 2008) ,
                        aes(xintercept = x), alpha = .7,
               linetype = 2) +
  geom_text(data = data.frame(x = as.Date("2008-09-16"), ANIODEF = 2008) ,
                        aes(x, 10,
                            label = "Prision riots"),
            hjust = 1.03, vjust = 0, size = 3.5) +
  #geom_vline(data = data.frame(x = as.Date("2009-01-15"), ANIODEF = 2009) ,
   #                     aes(xintercept = x), alpha = .7,
    #           linetype = 2) +
  #geom_text(data = data.frame(x = as.Date("2009-01-15"), ANIODEF = 2009) ,
   #                     aes(x, 10,
    #                        label = "Truce?"),
     #       hjust = -.15, vjust = 0, size = 3.5)  +
  geom_vline(data = data.frame(x = as.Date("2008-12-10"), ANIODEF = 2008) ,
                        aes(xintercept = x), alpha = .7,
               linetype = 2) +
  geom_text(data = data.frame(x = as.Date("2008-12-10"), ANIODEF = 2008) ,
                        aes(x, 10,
                            label = "Leyzaola"),
            hjust = -.05, vjust = 0, size = 3.5) +
  geom_vline(data = data.frame(x = as.Date("2009-06-15"), ANIODEF = 2009) ,
                        aes(xintercept = x), alpha = .7,
               linetype = 2) +
  geom_text(data = data.frame(x = as.Date("2009-06-15"), ANIODEF = 2009) ,
                        aes(x, 20,
                            label = "\"La Perra\" captured"),
            hjust = -.05, vjust = 0, size = 3.5) +
  geom_vline(data = data.frame(x = as.Date("2009-07-02"), ANIODEF = 2009) ,
                        aes(xintercept = x), alpha = .7,
               linetype = 2) +
  geom_text(data = data.frame(x = as.Date("2009-07-02"), ANIODEF = 2009) ,
                        aes(x, 15,
                            label = "\"El Marro\" captured"),
            hjust = -.05, vjust = 0, size = 3.5) +
   geom_line()
ggsave("graphs/tijuana-daily-select.png", height = 5, width = 13, dpi = 100)




hom.mexic <- subset(hom, MA == "Mexicali")
llcharts <- generateCharts(hom.mexic, kmaxy, "Mexicali")
saveCharts(llcharts, "mexicali")

hom.cul <- subset(hom, ENTOCU == 25 & MUNOCU %in% c(006, 018))
llcharts <- generateCharts(hom.cul, kmaxy, "Culiacán - Navolato")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2008-05-13")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2008-05-13"), 22,
                label = "J.O. Culiacan-Navolato"),
             hjust = 1.03, vjust = 0)
saveCharts(llcharts, "culiacán")
#chartRegion(hom, c(25), c(006), kmaxy,
#            "Culiacán")

hom.maz <- subset(hom, ENTOCU == 25 & MUNOCU %in% c(012))
llcharts <- generateCharts(hom.maz, kmaxy, "Mazatlán")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2008-07-15")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2008-07-15"), 8, label = "J.O. Culiacán - Navolato (Mazatlán)"),
             hjust = 1.03, vjust = 0)
saveCharts(llcharts, "mazatlán")
#chartRegion(hom, c(25), c(012), kmaxy,
 #           "Mazatlán", func)

hom.aca <- subset(hom, ENTOCU == 12 & MUNOCU %in% c(001, 021))
llcharts <- generateCharts(hom.aca, kmaxy, "Acapulco (MA)")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2007-01-15")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-01-15"), 20, label = "J.O. Acapulco"),
             hjust = 1.03, vjust = 0)
saveCharts(llcharts, "acapulco (ma)")



hom.zihua <- subset(hom, ENTOCU == 12 & MUNOCU %in% c(038))
llcharts <- generateCharts(hom.zihua, kmaxy, "Zihuatanejo")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2007-01-15")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-01-15"), 20, label = "J.O. Acapulco"),
             hjust = 1.03, vjust = 0)
saveCharts(llcharts, "zihuatanejo (ma)")

hom.zihua <- subset(hom, ENTOCU == 12 & MUNOCU %in% c(38))
llcharts <- generateCharts(hom.zihua, kmaxy, "Zihuatanejo")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2007-01-15")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-01-15"), 20, label = "J.O. Acapulco"),
             hjust = 1.03, vjust = 0)
saveCharts(llcharts, "zihuatanejo (ma)")

hom.cg <- subset(hom, ENTOCU == 12 & MUNOCU %in% c(068, 16, 73,
                           38, 22, 50, 27, 64, 67, 7, 3, 57, 54, 11, 14, 007,
                           21, 01))
llcharts <- generateCharts(hom.cg, kmaxy, "Costa Grande, Tierra Caliente and Acapulco")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2007-01-15")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-01-15"), 20, label = "J.O. Acapulco"),
             hjust = 1.03, vjust = 0)
saveCharts(llcharts, "costa grande (guerrero)")


hom.lar <- subset(hom, MA == "Nuevo Laredo")
llcharts <- generateCharts(hom.lar, kmaxy, "Nuevo Laredo (MA)")
llcharts$monthly <- llcharts$monthly +
  geom_vline(aes(xintercept = as.Date("2005-06-13")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2005-06-13"), 20, label = "Operation\nMexico Secure"),
             hjust = 1.03, vjust = 0) +
  geom_vline(aes(xintercept = as.Date("2007-02-19")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-02-19"), 20, label = "J.O. Tamaulipas - Nuevo León"),
             hjust = 0, vjust = 0)
saveCharts(llcharts, "nuevo laredo (ma)")
#chartRegion(hom, c(28), c(027), kmaxy,
 #           "Nuevo Laredo")
chartRegion(hom, c(28), c(022), kmaxy,
            "Matamoros (MA)")  
chartRegion(hom, c(28), c(032, 033), kmaxy,
            "Reynosa (MA)")

chartRegion(hom, c(26), c(043), kmaxy,
            "Nogales")

chartRegion(hom, c(05), c(004, 027, 030), kmaxy,
            "Saltillo (MA)")

chartRegion(hom, c(18), c(008, 017), kmaxy,
            "Tepic (MA)")

chartRegion(hom, c(06), c(007), kmaxy,
            "Manzanillo")


chartRegion(hom, c(16), c(053, 088), kmaxy,
            "Morelia (MA)")

hom.mon <- subset(hom, ENTOCU == 19 &
                  MUNOCU %in% c(6, 9, 018, 19, 21, 26, 31, 39,
                                45, 46, 48, 49))
llcharts <- generateCharts(hom.mon, kmaxy, "Monterrey (MA)")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2007-02-19")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-02-19"), 13, label = "J.O.\nTamaulipas - Nuevo León"),
             hjust = 1.03, vjust = 0)
llcharts$monthly <- llcharts$monthly +
  geom_vline(aes(xintercept = as.Date("2007-02-19")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-02-19"), 40, label = "J.O.\nTamaulipas - Nuevo León"),
             hjust = 1.03, vjust = 0) +
  geom_vline(aes(xintercept = as.Date("2009-10-04")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2009-10-04"), 40, label = "New Governor"),
             hjust = 1.03, vjust = 0)
saveCharts(llcharts, "monterrey (ma)")
#chartRegion(hom, c(19),
 #           c(6, 9, 018, 19, 21, 26, 31, 39, 45, 46, 48, 49),
  #          2008,
   #         "Monterrey (MA)")


hom.gua <- subset(hom, ENTOCU == 14 &
                  MUNOCU %in% c(39, 44, 51, 70, 97, 98, 101, 120))
llcharts <- generateCharts(hom.gua, kmaxy, "Guadalajara (MA)")
saveCharts(llcharts, "guadalajara (ma)")

hom.vall <- subset(hom, (ENTOCU == 14 & MUNOCU %in% c(067)) |
              (ENTOCU == 18 & MUNOCU %in% c(20)))
llcharts <- generateCharts(hom.vall, kmaxy, "Puerto Vallarta")
saveCharts(llcharts, "puerto_vallarta")

chartRegion(hom, c(10), c(005), kmaxy,
            "Durango")
chartRegion(hom, c(17), c(007, 008, 011, 018, 020, 028), kmaxy,
            "Cuernavaca")
chartRegion(hom, c(10), c(032), kmaxy,
            "Santiago Papasquiaro")

hom.lag <- subset(hom, (ENTOCU == 5 & MUNOCU %in% c(17,35)) |
              (ENTOCU == 10 & MUNOCU %in% c(7,12)))
llcharts <- generateCharts(hom.lag, kmaxy, "La Laguna")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2007-06-14")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-06-14"), 13, label = "Operation La Laguna"),
             hjust = 1.03, vjust = 0) +
  geom_vline(aes(xintercept = as.Date("2007-09-01")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-09-01"), 20, label = "Operation La Laguna (Expanded)"),
             hjust = 1.03, vjust = 0)
llcharts$monthly <- llcharts$monthly +
  geom_vline(aes(xintercept = as.Date("2007-06-14")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-06-14"), 30, label = "Operation La Laguna"),
             hjust = 1.03, vjust = 0) +
  geom_vline(aes(xintercept = as.Date("2007-09-01")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-09-01"), 50, label = "Operation La Laguna (Expanded)"),
             hjust = 1.03, vjust = 0)
saveCharts(llcharts, "laguna")

hom.lag <- subset(hom, (ENTOCU == 5 & MUNOCU %in% c(17,35)) )
llcharts <- generateCharts(hom.lag, kmaxy, "Torreón")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2007-06-14")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-06-14"), 10, label = "Operation La Laguna"),
             hjust = 1.03, vjust = 0) +
  geom_vline(aes(xintercept = as.Date("2007-09-01")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-09-01"), 15, label = "Operation La Laguna (Expanded)"),
             hjust = 1.03, vjust = 0)
saveCharts(llcharts, "torreon")

hom.ver <- subset(hom, ENTOCU == 30)
llcharts <- generateCharts(hom.ver, kmaxy, "Veracruz (State)")
llcharts$monthly <- llcharts$monthly +
  geom_vline(aes(xintercept = as.Date("2007-05-14")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-05-14"), 45, label = "J.O. Veracruz"),
             hjust = 1.1, vjust = 0)
saveCharts(llcharts, "veracruz")

hom.ver <- subset(hom, MA == "Coatzacoalcos")
llcharts <- generateCharts(hom.ver, kmaxy, "Coatzacoalcos (MA)")
llcharts$monthly <- llcharts$monthly +
  geom_vline(aes(xintercept = as.Date("2007-05-14")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-05-14"), 45, label = "J.O. Veracruz"),
             hjust = 1.1, vjust = 0)
saveCharts(llcharts, "coatzacoalcos (ma)")

hom.dur <- subset(hom, ENTOCU == 10)
llcharts <- generateCharts(hom.dur, kmaxy, "Durango (State)")
saveCharts(llcharts, "durango (state)")

hom.tam <- subset(hom, ABBRV == "Tamps")
llcharts <- generateCharts(hom.tam, kmaxy, "Tamaulipas (State)")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2007-02-19")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-02-19"), 13, label = "J.O.\nTamaulipas - Nuevo León"),
             hjust = 1.03, vjust = 0)
saveCharts(llcharts, "tamaulipas (state)")

hom.mich <- subset(hom, ABBRV == "Mich")
llcharts$monthly <- labelChart(llcharts$monthly, "Start of Drug War",
                              1100, op.mich)
saveCharts(llcharts, "mexico")




hom.tam <- subset(hom, ABBRV == "Tamps")
llcharts <- generateCharts(hom.tam, kmaxy, "Tamaulipas (State)")
llcharts$weekly <- llcharts$weekly +
  geom_vline(aes(xintercept = as.Date("2007-02-19")), alpha = .7,
               linetype = 2) +
  geom_text(aes(as.Date("2007-02-19"), 13, label = "J.O.\nTamaulipas - Nuevo León"),
             hjust = 1.03, vjust = 0)
saveCharts(llcharts, "tamaulipas (state)")

hom.mxc <- subset(hom, MA == "Valle de México")
llcharts <- generateCharts(hom.mxc, kmaxy, "Mexico City")
saveCharts(llcharts, "mexico city")

hom.slp <- subset(hom, ABBRV == "SLP")
llcharts <- generateCharts(hom.slp, kmaxy, "San Luis Potosí")
saveCharts(llcharts, "san luis potosi")
            
hom.mich <- subset(hom, ABBRV == "Mich")
llcharts <- generateCharts(hom.mich, kmaxy, "Michoacán (State)")
llcharts$weekly <- labelChart(llcharts$weekly, "J.O. Michoacán",
                              47, op.mich)
llcharts$weekly <- labelChart(llcharts$weekly, "J.O. Michoacán II",
                              40, "2009-07-01")
llcharts$weekly <- labelChart(llcharts$weekly, "La Minsa Captured",
                              47, "2009-07-11")
llcharts$monthly <- labelChart(llcharts$monthly, "J.O. Michoacán",
                              15, op.mich)
llcharts$monthly <- labelChart(llcharts$monthly, "J.O. Michoacán II",
                              120, "2009-07-11")
llcharts$monthly <- labelChart(llcharts$monthly, "La Minsa Captured",
                              15, "2009-07-11")
saveCharts(llcharts, "michoacan (state)")

hom.mich.mun <- subset(hom, ABBRV == "Mich" & MUNOCU %in% c(14, 26, 15,
                              89, 12, 8, 2, 6, 4,
                          55, 35, 96, 10, 52, 29, 38, 13, 92, 33))
llcharts <- generateCharts(hom.mich.mun, kmaxy, "Michoacán (Selected Municipalities)")
llcharts$weekly <- labelChart(llcharts$weekly, "J.O. Michoacán",
                              25, op.mich)
llcharts$weekly <- labelChart(llcharts$weekly, "J.O. Michoacán II",
                              21, "2009-07-01")
llcharts$weekly <- labelChart(llcharts$weekly, "La Minsa Captured",
                              25, "2009-07-11")
llcharts$monthly <- labelChart(llcharts$monthly, "J.O. Michoacán",
                              1, op.mich)
llcharts$monthly <- labelChart(llcharts$monthly, "J.O. Michoacán II",
                              1, "2009-07-11")
llcharts$monthly <- labelChart(llcharts$monthly, "La Minsa Captured",
                              5, "2009-07-11")
saveCharts(llcharts, "michoacan (selected)")

hom.zac <- subset(hom, ENTOCU == 32)
llcharts <- generateCharts(hom.zac, kmaxy, "Zacatecas (State)")
saveCharts(llcharts, "zacatecas (state)")

#Femicide rate in Mexico
#Mexico CONAPO
fmx <- ddply(subset(fem, ANIODEF %in% 2006:2009),
             .(ANIODEF), nrow) / c(53219640, 53723982, 54216256,
                                        54696909) * 100000
#US - FBI UNIFORM CRIME REPORTS
#http://www2.fbi.gov/ucr/cius2009/offenses/expanded_information/data/shrtable_01.html
fus <- c(3156, 3177, 3078, 3122) / c(151751781, 153102214,
                                      154446928, 154446928) * 100000

fhom.rates <- data.frame(year = factor(2006:2009),
                         US = fus,
                         MX = fmx$V1)
ggplot(melt(fhom.rates, id = "year"), aes(year, value, group = variable, color = variable)) +
  geom_line() +
  opts(title = "Female Homicide Rates in the US and Mexico (2006-2009)") +
  ylab("homicide rate per 100,000 women") +
  ylim(0, max(fhom.rates$MX)) +
  scale_colour_manual("Country",
                      values = c("blue", "darkgreen"))
ggsave("graphs/femicide-rates-us-mx.png", dpi = 100,
       width = 8, height = 6)

babies <- subset(hom, SEXO != 0 & EDADVALOR < 800)
ggplot(babies, aes(EDADVALOR, ..count.. ,group = SEXOtxt,
                   fill = SEXOtxt))+
  geom_histogram(position = "identity", binwidth = 1, alpha = .5)

chartRegion(hom, c(16), c(52), kmaxy,
            "Lázaro Cárdenas")  

#It might be worthwhile to include other states, counties, and
#also presumed homicides + violent deaths of unknown intent
#hom <- subset(hom, SEXO == 2)
#hom <- subset(hom, ENTOCU %in% c(19, 28))
#hom <- subset(hom, ENTOCU %in% c(9, 12, 15, 16, 17))
#hom <- subset(hom, ENTOCU %in% c(2, 8, 10, 18, 25, 26))
ddply(hom, .(ANIODEF), nrow)
#Total homicides in 2009
(19121 *  1/(13628/14175) ) / 1110
#write.csv(ddply(hom, .(ANIODEF, MESDEF), nrow), "monthly.csv")

ddply(hom.j, .(ANIODEF), nrow)
2316*1610/1570
#write.csv(ddply(hom, .(ANIODEF, ABBRV), nrow), "temp.csv")
sd(subset(hom, ANIODEF == 2009 & EDADVALOR < 120)$EDADVALOR)

kids <- subset(hom, EDADVALOR >= 60)
ddply(kids, .(ANIODEF), nrow)
